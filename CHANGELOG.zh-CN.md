...
- **全局搜索大纲更新** — 全局搜索打开时刷新一次大纲并跟随大纲管理器更新，不再每 1200ms 轮询，减少搜索面板打开时的后台工作。(#692)
- **会话侧边栏同步** — 会话同步现在会把新增、更新、删除和目标文件夹变化批量写入一次 store，减少历史同步时的存储写入。(#697)
- **后台页面工作量** — 页内收藏图标现在会避开无关 DOM 变化触发的完整大纲扫描，选中的提示词复用统一 URL 变化监听；标签页标题检查只在页面后台空闲时降频，同时保留生成完成通知入口。
- **千问长会话 DOM 适配** — 适配千问对话页新版长会话懒加载结构，导出会话和复制大纲现在会先补齐早期历史再收集消息。

### 🐛 问题修复

- **提示词发送快捷键** — 修复在 Claude、DeepSeek 等站点输入框中使用 Ophel 配置的发送快捷键时可能出现明显延迟的问题；双击发送提示词现在也会遵守 Ctrl+Enter 发送模式，不再把提示词留在输入框或插入换行。(#701, #710)
- **ChatGPT 置顶会话** — 修复 ChatGPT 新侧边栏布局下的会话同步问题；“已置顶”分区中的会话现在会被导入并跟踪标题变化，项目文件夹不会被误识别为会话。
- **Codex task 页面大纲** — ChatGPT 上的 Codex Cloud task 分享页现在会在大纲中显示原始问题和回复标题，不再显示为空。(#709)
- **Gemini 会话同步** — 修复 Gemini 首次安装或会话库为空时自动同步可能只加载部分历史、会话面板数量仍显示为空，以及手动同步滚到底后等待过久的问题。(#712)
- **Gemini 分享对话继续按钮** — 修复净化模式可能隐藏 Gemini 分享页「Continue this chat」按钮的问题。(#703)
- **豆包长对话导出** — 修复豆包虚拟滚动长对话可能只导出或复制当前挂载消息窗口的问题；导出和复制完整大纲现在会先扫描对话再收集消息。
- **大纲滚动高亮稳定性** — 修复正文滚动时可能高亮用户问题、上一条回答或隐藏子标题，而不是当前可见标题的问题。(#704, #708)
...
- **快捷引用链（Quick Quote Chains）** — 提示词链现在可直接从快捷引用操作触发，支持链图标选择器、拖拽排序和内嵌引用锚点，确保页面刷新后可靠恢复引用定位。(#623)
- **Gemini Canvas Markdown 操作** — Gemini Canvas 工具栏现在新增 Ophel 复制 Markdown 和下载 Markdown 按钮，可将 Canvas 代码导出为带代码块的 Markdown，不必执行完整会话导出。(#663)
- **分段对话导出** — 对话导出现在支持在可搜索选择器中选择消息范围，并将选中片段导出为 ZIP、合并 Markdown 或复制到剪贴板。(#665)
- **更新说明弹窗** — Ophel 现在会在每个版本首次打开时展示更新说明，也可从关于页面重新查看；扩展版和油猴版都能看到本次更新内容。(#666)

### ✨ 功能优化

- **快捷引用原生站点定位** — 快捷引用操作现在会检测支持站点的原生选中文本弹窗，并自动选择不重叠的位置；在会与原生编辑器冲突的页面上也会停用 Ophel 锚点。(#658, #660, #661)
- **快捷引用默认操作** — “引用回复”现在作为默认提示词链提供，选中文本弹窗保留固定复制操作，并减少内置操作数量。(#664)
- **提示词链启用与同步** — 从选中文本触发提示词链时，现在会在需要时提示启用提示词队列；提示词链面板也会提示启用快捷引用，油猴版备份和 WebDAV 同步会包含链数据。(#661, #671)
- **商店与工具栏图标** — 更新 Chrome 商店图标，弱化评价图标悬浮效果，并让大纲工具栏图标颜色与其他标签保持一致。(#662, #668)
...
- **千问 Slate 编辑器状态同步** — 修复从提示词队列插入内容后无法编辑删除的问题。编辑器现在通过 `全选 + 删除 + 插入文本` 并触发完整的事件链（`beforeinput`、`input`、`change`）来正确同步 Slate 的虚拟 DOM，确保 DOM 和 Slate 状态同步更新。
- **启动器预览保留逻辑** — 鼠标从面板 Logo 移动到其他快捷按钮（如工具箱）时，面板悬浮预览现在会自动收起，不再遮挡工具箱菜单。(#653)
- **快捷引用导出标记** — 修复不可见快捷引用锚点可能在对话导出和大纲复制中留下空大纲条目，或导致消息被过滤的问题。(#669)
- **油猴版重置与恢复状态** — “清除全部数据”、备份恢复和更新说明状态现在都会使用统一的平台存储键，避免油猴版重置不完整或恢复状态残留。(#670)
